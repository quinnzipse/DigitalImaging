...
public class RunLengthEncoder extends ImageEncoder {
    ...
    @Override
    public void writeHeader(BufferedImage source, DataOutput output) throws IOException {
        output.writeUTF(this.getMagicWord());
        output.writeShort((short)source.getWidth());
        output.writeShort((short)source.getHeight());
        output.writeInt(source.getColorModel().getColorSpace().getType());
    }
    ...
    private int getRun(BufferedImage source, int col, int row, int band, int bit, boolean isWhite) {
        ...
        while (col < source.getWidth() &&
                getBit(source.getRaster().getSample(col, row, band), bit) == isWhite) {
            col++;
            result++;
        }
